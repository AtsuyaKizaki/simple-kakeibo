          ...
          <table class="table table-fixed">
            ...
            <tbody>
              <?php foreach($records as $record): ?>
                <tr>
                  <td class="col-2"><?= h($record['date']);?></td>
                  <td class="col-3"><?= h($record['title']);?></td>
                  <td class="col-2"><?= h($record['type']) == 0 ? h($record['amount']): ''?></td>
                  <td class="col-2"><?= h($record['type']) == 1 ? h($record['amount']): ''?></td>
                  <td class="col-3">
                    <a href="./editForm.php?id=<?= h($record['id']); ?>" class="btn btn-success text-light">編集</a>
                    <a href="./delete.php?id=<?= h($record['id']); ?>" class="btn btn-danger text-light">削除</a>
                  </td>
                </tr>
              <?php endforeach; ?>
            </tbody>
